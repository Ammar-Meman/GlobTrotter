...
export default function AuthPage({ initialMode = "login" }) {
  ...
  const InfoPanel = () => (
    <motion.div
      key={isSignup ? "signup-info" : "login-info"}
      initial={{ opacity: 0, x: isSignup ? 60 : -60 }}
      animate={{ opacity: 1, x: 0 }}
      exit={{ opacity: 0, x: isSignup ? -60 : 60 }}
      transition={{ duration: 0.5, ease: "easeInOut" }}
      className="p-4 sm:p-8 flex flex-col justify-between space-y-8"
    >
      <Logo size="lg" to={null} />

      <div className="space-y-4 max-w-xl">
        <h1 className="text-5xl sm:text-6xl lg:text-[56px] font-extrabold text-white tracking-tight leading-[1.1] drop-shadow-lg">
          Plan. Explore. <br />
          <span className="text-blue-400 drop-shadow-md">Experience.</span>
        </h1>
        <p className="text-white/90 text-base sm:text-lg font-medium leading-relaxed max-w-lg drop-shadow-sm">
      ...
      <div className="space-y-4 max-w-lg pt-2">
        {features.map((item, i) => {
          ...
          return (
            ...
            >
              <div className="w-12 h-12 rounded-2xl bg-white/20 backdrop-blur-sm flex items-center justify-center shrink-0 border border-white/30 transition-transform group-hover:scale-105">
                <Icon className="w-6 h-6 text-white" />
              </div>
              <div className="space-y-0.5">
                <h4 className="font-bold text-base text-white drop-shadow-sm">{item.title}</h4>
  ...
  const FormCard = () => (
    <motion.div
      ...
      exit={{ opacity: 0, x: isSignup ? 60 : -60 }}
      transition={{ duration: 0.5, ease: "easeInOut" }}
      className="bg-white rounded-[32px] p-8 sm:p-12 shadow-[0_25px_70px_-15px_rgba(0,0,0,0.25)] border border-slate-100 flex flex-col justify-between space-y-6"
    >
      {/* Header */}
        ...
        <form onSubmit={handleSubmitLogin(onLoginSubmit)} className="space-y-5">
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input id="email" type="email" placeholder="Enter your email" autoComplete="email"
                {...registerLogin("email")}
                className={`pl-10 h-12 text-sm bg-white border-slate-200 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 rounded-xl text-slate-800 ${errorsLogin.email ? "border-red-500" : ""}`}
              />
            </div>
            {errorsLogin.email && <p className="text-[11px] text-red-500 font-medium">{errorsLogin.email.message}</p>}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input id="password" type={showPassword ? "text" : "password"} placeholder="Enter your password" autoComplete="current-password"
                {...registerLogin("password")}
                className={`pl-10 pr-10 h-12 text-sm bg-white border-slate-200 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 rounded-xl text-slate-800 ${errorsLogin.password ? "border-red-500" : ""}`}
              />
              <button type="button" onClick={() => setShowPassword(!showPassword)}
                className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 cursor-pointer" tabIndex={-1}>
                {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
              </button>
            </div>
            {errorsLogin.password && <p className="text-[11px] text-red-500 font-medium">{errorsLogin.password.message}</p>}
          </div>
        ...
        <form onSubmit={handleSubmitSignup(onSignupSubmit)} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input id="signup-name" type="text" placeholder="Enter your full name" autoComplete="name"
                {...registerSignup("name")}
                className={`pl-10 h-11 text-sm bg-white border-slate-200 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 rounded-xl text-slate-800 ${errorsSignup.name ? "border-red-500" : ""}`}
              />
            </div>
            {errorsSignup.name && <p className="text-[11px] text-red-500 font-medium">{errorsSignup.name.message}</p>}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input id="signup-email" type="email" placeholder="Enter your email" autoComplete="email"
                ...
                className={`pl-10 h-11 text-sm bg-white border-slate-200 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 rounded-xl text-slate-800 ${errorsSignup.email ? "border-red-500" : ""}`}
              />
            </div>
            {errorsSignup.email && <p className="text-[11px] text-red-500 font-medium">{errorsSignup.email.message}</p>}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <Lock className="w-4 h-4 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2" />
              <Input id="signup-password" type={showPassword ? "text" : "password"} placeholder="Create a password" autoComplete="new-password"
                {...registerSignup("password")}
                className={`pl-10 pr-10 h-11 text-sm bg-white border-slate-200 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 rounded-xl text-slate-800 ${errorsSignup.password ? "border-red-500" : ""}`}
              />
              <button type="button" onClick={() => setShowPassword(!showPassword)}
                className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 cursor-pointer" tabIndex={-1}>
                {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
            ...
            {errorsSignup.password && <p className="text-[11px] text-red-500 font-medium">{errorsSignup.password.message}</p>}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <Lock className="w-4 h-4 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2" />
              <Input id="signup-confirm" type={showConfirmPassword ? "text" : "password"} placeholder="Confirm your password" autoComplete="new-password"
                {...registerSignup("confirmPassword")}
                className={`pl-10 pr-10 h-11 text-sm bg-white border-slate-200 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 rounded-xl text-slate-800 ${errorsSignup.confirmPassword ? "border-red-500" : ""}`}
              />
            ...
            {errorsSignup.confirmPassword && <p className="text-[11px] text-red-500 font-medium">{errorsSignup.confirmPassword.message}</p>}
          </div>

          <div className="flex items-start gap-2 pt-1">
            <input id="agreeTerms" type="checkbox" {...registerSignup("agreeTerms")}
          ...
          {errorsSignup.agreeTerms && <p className="text-[11px] text-red-500 font-medium">{errorsSignup.agreeTerms.message}</p>}
        ...
        {!isSignup ? (
          <p className="text-xs text-slate-500 font-medium">
            ...
            <button type="button" onClick={() => toggleMode(true)}
              className="font-bold text-blue-600 hover:text-blue-700 hover:underline cursor-pointer ml-1">Sign up</button>
          </p>
        ) : (
          <p className="text-xs text-slate-500 font-medium">
            Already have an account?{" "}
            <button type="button" onClick={() => toggleMode(false)}
              className="font-bold text-blue-600 hover:text-blue-700 hover:underline cursor-pointer ml-1">Login</button>
          </p>
        )}
      </div>
    </motion.div>
  ...
  return (
    <div className="min-h-screen w-full relative flex items-center justify-center p-4 sm:p-6 lg:p-12 bg-slate-950 overflow-hidden font-sans select-none">
      ...
      {/* Background Images with Reduced Opacity for Readability */}
      <div
        className={`absolute inset-0 bg-cover bg-center transition-opacity duration-1000 ease-in-out ${
          !isSignup ? "opacity-60" : "opacity-0"
        }`}
        style={{
          backgroundImage:
      ...
      {/* Subtle Dark Gradient Overlay for Ambient Contrast */}
      <div className="absolute inset-0 bg-gradient-to-t from-slate-950/60 via-slate-950/20 to-slate-950/30" />

      {/* Main Container */}
      <div className="relative z-10 w-full max-w-7xl mx-auto min-h-[660px] flex items-center">
        <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-14 items-center">

          {/* Animated Layout - panels swap positions */}
          <AnimatePresence mode="wait">
            {!isSignup ? (
              <>
                <div key="login-info-slot" className="lg:order-1">
                  <InfoPanel />
                </div>
                <div key="login-form-slot" className="lg:order-2">
                  <FormCard />
                </div>
              </>
            ) : (
              <>
                <div key="signup-form-slot" className="lg:order-1">
                  <FormCard />
                </div>
                <div key="signup-info-slot" className="lg:order-2">
                  <InfoPanel />
                </div>
              </>
            )}
